...
`position: fixed` solves the overflow problem - it positions the element relative to the **viewport**, not the nearest positioned ancestor. But it introduces a new one: the coordinates are calculated once on open and never updated. **If the trigger moves (via scrolling, layout shifts, or resizing), the popover stays frozen in place.**

The approach I landed on combines three things: `position: fixed` to escape overflow, a **React Portal** to escape ancestor DOM constraints (like CSS `transform` creating a new containing block), and **dynamic repositioning** to keep the content anchored to the trigger.
...
The simplest case - when the browser window is resized, everything shifts ([`useWindowResize`](/lab/hooks/useWindowResize)):
...
This runs every frame while the popover is open and stops when it closes. Sounds more expensive than it is - it runs only for expanded instances (which usually is just one at a time) and it catches **every** type of movement - CSS transitions, sibling DOM changes, flex/grid reflows, you name it (see how in [`usePositionObserver`](/lab/hooks/usePositionObserver)):
...
For a popover, focus management is more or less straight forward - it gets quite trickier when popover gets extended into a dropdown or a select.
...
The trigger needs `aria-expanded` and `aria-describedby` pointing to the popover content's `id` (generated with `useId`). Without these, screen readers won't know the trigger controls a popover.
...
Instead of wrapping the trigger in an extra `<span>`, the [`Slot`](https://github.com/Ninjaneer87/andrejground-lab/blob/main/packages/lab/src/components/utility/Slot/index.tsx) component merges its props (event handlers, refs, aria attributes) directly onto the trigger's child element. This avoids extra DOM nodes, improves keyboard focus experience, keeps styles intact, and the trigger retains its original element type:
