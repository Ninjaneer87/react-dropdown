...
It's as simple as: 
- if it's **not** wrapped in the root context provider (`dropdownRootContext === null`), it's the root dropdown → Wrap it in the provider.
- if it **is** already wrapped in the root context provider, it's a child dropdown → Do not wrap it again.
...
When the root closes - click-outside, Escape, selection - `isRootOpen` flips to `false`, and every nested dropdown closes itself (with transitions). No event bus, no imperative refs, no prop drilling. The same `Dropdown` component behaves correctly at any depth purely based on what's in context.
...
- **`'none'`** - no auto-focus. The dropdown opens but focus stays on the trigger. Useful for hover-triggered dropdowns where stealing focus would be disorienting.
...
Set `focusTriggerOnClose={false}` on nested informational popovers inside menu items when hover/tooltip behavior should not participate in focus restoration.

:::info[Takeaway]
Coordinate open and close focus explicitly. Root overlays own outside-click focus restoration; leaf components like Dropdown should restore focus only when the user actually dismissed that menu.
:::
...
Dropdowns with hundreds of options (countries, users, products) can't load everything upfront. `DropdownSection` supports an `infiniteScrollProps` config that wires up an `IntersectionObserver` in [`useInfiniteScroll`](/lab/hooks/useInfiniteScroll) hook to a sentinel at the bottom of the scrollable area:
...
The observer fires `onLoadMore` when the sentinel enters the scrollable viewport. Combined with the `MutationObserver` in `useKeyboardNavigation`, newly loaded items are automatically keyboard-navigable - no extra wiring.
